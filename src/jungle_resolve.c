	...
	for (k = kh_begin(h); k != kh_end(h); ++k){
		if (kh_exist(h, k)){
			...
			if (is_source_edge(g, key, comp_set) && is_sink_edge(g, key, comp_set)){
				log_info(KRED "Self loop detect e.i is both source and sink %d" RESET, key);
				return 1;
			}
		}
	}
	...
	for (i = q->p; i < q->n; i++) {
		...
		for (j = 0; j < v[dest].deg; j++) {
			if (kh_get(khInt, set_v, v[dest].adj[j]) == kh_end(set_v)){
				log_info("%d - One remain node* have outgoing edge* or region is too complex", e_i);
				return 0;
			}
		}
	}
			...
			gint_t *bx_bin)
{
	...
	for (j = 0; j < 4; ++j) {
		for (i = j + 1; i < 4; ++i) {
			...
			log_info("Test connection %ld <-> %ld: %s%d" RESET,
	...
	do {
		...
		log_info(KCYN "One pair %d - %d" RESET, e1, e2);
		if (g->edges[e1].source == -1 || e1 == e2 || e1 == e_rc2)
			return 0;
...
static void polish_link(struct asm_graph_t *g0, gint_t *fw_link, gint_t *rv_link)
{
	...
	for (i = 0; i < g0->n_e; ++i){
		if (fw_link[i] == fw_link[g0->edges[i].rc_id] && fw_link[i] != 0){
			log_info(KGRN "Duplicated %d - %d - %d" RESET, i, g0->edges[i].rc_id,
					fw_link[i]);
		...
		gint_t ei, khash_t(khInt) *set)
{
	...
	while (e_it != 0) {
		printf("%d-", e_it);
		log_info(KCYN "%d - %d" RESET, ei, e_it);
...
void detect_simple_tandem(struct asm_graph_t *g0)
{
	...
	for (i = 0; i < g0->n_e; ++i){
		...
		if (simple_tandem(g0, i, &comp_sz, lg, comp_set, is_visited)){
			...
			log_info(KGRN "Complex Tandem %d" RESET, i);
			log_info(KBLU "Numer of keys %d" RESET, kh_size(lg));
			log_info(KMAG "Numer of edges in the complex %d" RESET, kh_size(comp_set));
			log_info(KWHT "Size of the complex %d" RESET, comp_sz);
			if (kh_size(lg) <= MAX_NUMBER_LEGS){
				if (is_large_loop(g0, lg, comp_set)){
					log_info(KRED "Is a self loop complex" RESET);
	...
	log_info("Number of resolved jungle: %d", cnt);
}
