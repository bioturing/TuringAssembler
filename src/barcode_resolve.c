		...
		khash_t(gint) *set_e, gint_t se, gint_t *adj, gint_t deg)
{
	...
	for (j = 0; j < deg; ++j) {
		...
		if (e == se || e == se_rc)
			continue;
		if (get_distance(g, min_contig_size, set_e,
					...
					gint_t se, gint_t *adj, gint_t deg)
{
	...
	if (second_best != -1 && !__strictly_greater(ret_ratio, second_best)) {
		__VERBOSE("Best = %.6lf; second best = %.6lf\n", ret_ratio, second_best);
		return -2;
	}
	return ret_e;
}
...
gint_t bc_find_consecutive_strict(struct asm_graph_t *g, gint_t se, gint_t *adj,
								gint_t deg)
{
	gint_t j, e, se_rc, ret_e;
	se_rc = g->edges[se].rc_id;
	...
	for (j = 0; j < deg; ++j) {
		e = adj[j];
		if (e == se || e == se_rc ||
			g->edges[se].source != g->nodes[g->edges[e].source].rc_id)
			continue;
		double ratio = get_barcode_ratio(g, se, e);
		if (ratio > ret_ratio) {
			second_best = ret_ratio;
				...
				khash_t(gint) *set_leg, double uni_cov)
{
	...
	do {
		...
		for (i = 0; i < n_leg; ++i) {
			...
			if (rcov2.hi != 1)
				continue;
			et1 = bc_find_pair_check_path_strict(g, set_e, e2, legs, n_leg);
			if (et1 != e1) {
				__VERBOSE("Not best pair (%ld, %ld) <-> %ld\n",
					e1, et1, e2);
				continue;
...
gint_t collapse_simple_jungle_strict(struct asm_graph_t *g)
{
	...
	__VERBOSE("Number of pair-edge join through jungle: %ld\n", ret);
	return ret;
}
