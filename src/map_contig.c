...
{
	mct->kmers = (khash_t(kmer_int) **) calloc(mct->n_candidates,
			sizeof(khash_t(kmer_int) *));
...
int count_match_kmer(khash_t(kmer_int) *first, khash_t(kmer_int) *second)
{
	...
	for (khiter_t it = kh_begin(first); it != kh_end(first); ++it){
		if (!kh_exist(first, it))
			continue;
		khiter_t it2 = kh_get(kmer_int, second, kh_key(first, it));
...
{
	for (int i = 0; i < mct->n_candidates; ++i)
		kh_destroy(kmer_int, mct->kmers[i]);
	free(mct->kmers);
	free(mct->is_match);
}
		...
		struct subseq_pos_t *local)
{
	...
	khash_t(kmer_int) *global_start, *global_end;
	...
	for (khiter_t it = kh_begin(local_kmers); it != kh_end(local_kmers);
		...
		it2 = kh_get(kmer_int, global_end, key);
	...
	for (int i = 0; i + WINDOW_SIZE <= (int) best_match.seq_len; ++i){
		...
		if (i > 0){
			...
			it = kh_get(kmer_int, global_end, pre_hash);
		...
		if (i + WINDOW_SIZE <= (int) best_match.seq_len){
			...
			it = kh_get(kmer_int, global_end, next_hash);
			if (it != kh_end(global_end)
	...
	kh_destroy(kmer_int, local_kmers);
	free(start_point);
	free(end_point);
}
...
void add_kmer(uint64_t hash, khash_t(kmer_int) *kmers)
{
	...
	if (it == kh_end(kmers)){
		int ret;
		it = kh_put(kmer_int, kmers, hash, &ret);
		kh_val(kmers, it) = 0;
	}
	++kh_val(kmers, it);
}
