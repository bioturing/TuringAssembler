...
void print_usage(const char *prog)
{
	...
	__VERBOSE("          stream [options] -1 read_1_added.fq -2 read_2_added.fq -l bioturing -o out_dir\n");
}
...
void smart_load_process(struct opt_proc_t *opt)
{
	__VERBOSE("Try to get stream\n");
...
int main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "sort"))
		...
		build_opt_process(argc, argv, &smart_load_process);
	else
		return -1;
	return 0;
}
