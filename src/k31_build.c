...
static inline uint32_t *k31_init_binseq(k31key_t key, int l)
{
	k31key_t tmp;
	__k31_rev_num(key, tmp, l);
			...
			struct k31hash_t *kmer_hash, struct asm_graph_t *ret_g)
{
	...
	ndict = calloc(1, sizeof(struct k31_idhash_t));
	k31_convert_table(kmer_hash, edict);
	k31hash_destroy(kmer_hash);

	k31_idhash_init(ndict, opt->hash_size - 1, 0);
	__VERBOSE("|----Condensing node and edge\n");
	k31_internal_build(ksize, edict, ndict, ret_g);
	test_asm_graph(ret_g);

	__VERBOSE("|----Estimating edges coverage\n");
	k31_count_edge(opt, ret_g, edict, ndict);
	__VERBOSE("\n");
	k31_idhash_clean(edict);
	k31_idhash_clean(ndict);
	free(edict);
	free(ndict);
}
			...
			struct k31_idhash_t *ndict, struct asm_graph_t *g)
{
	...
	int deg_fw, deg_rv;
	int lmc = (ksize << 1) - 2;
	k31key_t kmask = ((k31key_t)1 << (ksize << 1)) - 1;
	gint_t n_v, n_e;
	...
	for (i = 0; i < edict->size; ++i) {
		if (IDHASH_KEY(edict, i) == K31_NULL)
			continue;
		...
		if ((deg_fw == 1 && deg_rv == 1) || deg_fw + deg_rv == 0)
			continue;
		kmint_t k = k31_idhash_put(ndict, IDHASH_KEY(edict, i));
	...
	for (i = 0; i < ndict->size; ++i) {
		if (IDHASH_KEY(ndict, i) == K31_NULL)
			continue;
		k31key_t node_knum, node_krev, cur_knum, cur_krev;
		gint_t id_fw, id_rv;
		int c, cur_c;
		uint32_t *e_seq;
		uint32_t e_len;
		...
		kmint_t k = k31_idhash_get(edict, node_knum);
		...
		for (c = 0; c < 4; ++c) {
			if (!((adj_fw >> c) & 1))
				continue;
			e_seq = k31_init_binseq(node_knum, ksize);
			e_len = ksize;
			cur_knum = node_knum;
			cur_krev = node_krev;
			cur_c = c;
			do {
				...
				kmint_t j;
				if (cur_knum < cur_krev) {
					k = k31_idhash_get(ndict, cur_knum);
					if (k == IDHASH_END(ndict)) {
						j = k31_idhash_get(edict, cur_knum);
					...
					k = k31_idhash_get(ndict, cur_krev);
					if (k == IDHASH_END(ndict)) {
						j = k31_idhash_get(edict, cur_krev);
						assert(j != IDHASH_END(edict));
						assert(__bin_degree4(
		...
		for (c = 0; c < 4; ++c) {
			if (!((adj_rv >> c) & 1))
				continue;
			e_seq = k31_init_binseq(node_krev, ksize);
			e_len = ksize;
			cur_knum = node_krev;
			cur_krev = node_knum;
			cur_c = c;
...
static void inc_count_edge(k31key_t knum, k31key_t krev, int c, struct asm_graph_t *g,
		struct k31_idhash_t *edict, struct k31_idhash_t *ndict)
{
	kmint_t e_pos, n_pos;
	gint_t e_id, e_id_rc, n_id;
	if (knum < krev) {
		n_pos = k31_idhash_get(ndict, knum);
		if (n_pos != IDHASH_END(ndict)) {
			...
			e_pos = k31_idhash_get(edict, knum);
		...
		n_pos = k31_idhash_get(ndict, krev);
		if (n_pos != IDHASH_END(ndict)) {
			...
			e_pos = k31_idhash_get(edict, krev);
			if (e_pos == IDHASH_END(edict))
				return;
			e_id = IDHASH_ID(edict, e_pos);
			atomic_add_and_fetch64(&(g->edges[e_id].count), 1);
