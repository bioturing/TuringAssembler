...
khash_t(kmer_int) *get_kmer_hash(char *r1_path, char *r2_path, int ksize)
{
	...
	for (int i = 0; i < rlist1.n; ++i){
		count_hash_from_seq(rlist1.reads[i].seq, ksize, res);
		char *rv = (char *) calloc(rlist1.reads[i].len + 1, sizeof(char));
		strcpy(rv, rlist1.reads[i].seq);
		flip_reverse(rv);
		count_hash_from_seq(rv, ksize, res);
		free(rv);
	}
	for (int i = 0; i < rlist2.n; ++i){
		count_hash_from_seq(rlist2.reads[i].seq, ksize, res);
...
void count_hash_from_seq(char *seq, int ksize, khash_t(kmer_int) *h)
{
	...
	uint64_t hash = 0;
	for (int i = 0; i < ksize - 1; ++i)
		hash = hash * BASE + base_to_int(seq[i]);
	int len = strlen(seq);
	for (int i = 0; i + ksize <= len; ++i){
		if (i > 0)
			hash -= power * base_to_int(seq[i - 1]);
		hash = hash * BASE + base_to_int(seq[i + ksize - 1]);
...
int count_kmer_on_seq(khash_t(kmer_int) *h, char *seq, int ksize)
{
	...
	count_hash_from_seq(seq, ksize, h1);
	int res = 0;
		...
		khash_t(kmer_int) *kmer_count)
{
	...
	count_hash_from_seq(join, check_ksize, h);
	int res = 1;
