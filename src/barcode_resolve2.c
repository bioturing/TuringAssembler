...
void resolve_1_2(struct asm_graph_t *g, struct opt_proc_t *opt) {
	int *total_len = calloc(1000, 4);
	int total_len_have_loop = 0;
	...
	partition_graph(ori_read, g, partition, opt->n_threads, opt->mmem, &partition_kmer_count, &n_partitions);
	for (int i_e = 0; i_e < g->n_e; i_e++) {
		int res = resolve_using_big_kmer(g, i_e, partition, partition_kmer_count);
		if (res==-1) {
			continue;
		}
		if (g->edges[i_e].seq_len < 1000 ) {
			total_len[g->edges[i_e].seq_len] += g->edges[i_e].seq_len - 75;
			total_len_have_loop += g->edges[i_e].seq_len - 75;
//			printf("edge %d is bridge 2-2\n", i_e);
		}
	}
	log_warn("total len middle edge resolve 2-2 %d have loop %d", total_len, total_len_have_loop);
	for (int i = 0 ;  i< 1000; i ++) {
		log_warn("aaa %d:%d", i, total_len[i]);
	}
	free(partition);
}
