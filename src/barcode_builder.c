...
void region_sample_mapper(struct read_t *r1, struct read_t *r2, uint64_t bc,
	...
	if ((ar1.n > 0 || ar2.n > 0) && bc != (uint64_t) -1){
		pthread_mutex_lock(bundle->lock);
		barcode_hash_add(bundle->h, bc);
...
void *region_sample_buffer_iterator(void *data) {
	struct region_sample_bundle_t *bundle = (struct region_sample_bundle_t *) data;
	struct dqueue_t *q = bundle->q;
	struct read_t read1, read2;
	struct pair_buffer_t *own_buf, *ext_buf;
	own_buf = init_pair_buffer();
	...
	while (1) {
		...
		while (1) {
			...
			region_sample_mapper(&read1, &read2, barcode, bundle);
